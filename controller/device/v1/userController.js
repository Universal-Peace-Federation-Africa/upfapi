...
const addUser = async (req, res) => {
  try {
    ...
    dataToCreate = {
      ...{ 'username':(req && req.user && req.user.id ? req.user.id.toString() : null) },
      ...dataToCreate,
    };
...
const bulkInsertUser = async (req, res)=>{
  try {
    ...
    const defaultValues = { 'username':(req && req.user && req.user.id ? req.user.id.toString() : null) };
    if (dataToCreate !== undefined && dataToCreate.length){
      ...
      if (createdUser && createdUser.length){
        ...
        if (userRoleData.length){
          await dbService.createMany(UserRole,userRoleData);
        }
      }
